...
from ..matcher import MatcherTopN
from ..postprocessing import detector_postprocess
from .build import META_ARCH_REGISTRY

__all__ = ["RetinaNetNoisy"]
...
def permute_all_cls_and_box_to_N_HWA_K_and_concat(box_cls, box_delta, num_classes=80):
    ...
    # here we keep the predictions seperated 
    box_cls = cat(box_cls_flattened, dim=1)
    box_delta = cat(box_delta_flattened, dim=1)
...
class RetinaNetNoisy(nn.Module):
    """
    ...
    def __init__(self, cfg):
        ...
        self.matcher = MatcherTopN(
            cfg.MODEL.RETINANET.NOISY.TOPN
        )
        self.alpha = cfg.MODEL.RETINANET.NOISY.ALPHA
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            losses = self.losses(gt_classes, gt_anchors_reg_deltas, box_cls, box_delta, soft_labels, reweight_coeffs)
    ...
    def losses(self, gt_classes, gt_anchors_deltas, pred_class_logits, pred_anchor_deltas, soft_labels, reweight_coeffs):
        """
        Args:
            ...
            For `soft_labels` and `reweight_coeffs`, see 
                :meth:`RetinaNetNoisy.get_ground_truth`.
        ...
        pred_class_logits = pred_class_logits.view(-1, self.num_classes) 
        pred_anchor_deltas = pred_anchor_deltas.view(-1, 4)

        gt_classes = gt_classes.flatten()
        gt_anchors_deltas = gt_anchors_deltas.view(-1, 4)
        soft_labels = soft_labels.flatten()
        reweight_coeffs = reweight_coeffs.flatten()
        ...
        # re-weight classification and localization losses
        # keep loss scale unchanged after re-weighting the loss
        loss_scale_cls = loss_cls.sum().data / (loss_cls * reweight_coeffs[:, None]).sum().data
        loss_scale_reg = loss_box_reg.sum().data / (loss_box_reg * reweight_coeffs[foreground_idxs, None]).sum().data
        loss_cls *= reweight_coeffs[:, None] * loss_scale_cls
        loss_box_reg *= reweight_coeffs[foreground_idxs, None] * loss_scale_reg
    ...
    @torch.no_grad()
    def get_ground_truth(self, anchors, targets, box_cls, box_delta):
        """
        Get ground truth as well as Soft Labels and Re-weighting Coefficients.
        Args:
            ...
            box_cls (list[Tensor]): network predictions from classification branch.
            box_delta (list[Tensor]): network predictions from bbox regression branch.
        Returns:
            gt_classes (Tensor):
            ...
            soft_labels (Tensor): 
                Soft labels for training the classification branch.
            reweight_coeffs (Tensor):
                Re-weighting coefficients to modulate the losses for classification and localization.
            """
        ...
        for i, (anchors_per_image, targets_per_image) in enumerate(zip(anchors, targets)):
            ...
            reweight_coeffs.append(reweight_coeffs_this)

        return torch.stack(gt_classes), torch.stack(gt_anchors_deltas), \
               torch.stack(soft_labels), torch.stack(reweight_coeffs)
